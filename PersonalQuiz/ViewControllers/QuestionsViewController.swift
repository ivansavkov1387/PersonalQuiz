...
class QuestionsViewController: UIViewController {
    ...
    // Transfer the array of choosed answers to ResultsViewController
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        guard segue.identifier == "resultSegue" else { return }
        guard let resultVC = segue.destination as? ResultsViewController else { return }
        resultVC.answersChosenTemp = answersChoosen
    }
    ...
    @IBAction func rangedAnswerButtonPressed() {
        //Logic for determining the index according to the range. New diapozones - see: showRangedAnswers
        answersChoosen.append(currentAnswers[lrintf(rangedSlider.value)])
...
extension QuestionsViewController {
    ...
    private func showRangedAnswers(with answers: [Answer]) {
        // Set new diapozon values.
        rangedSlider.minimumValue = 0
        rangedSlider.maximumValue = 3
